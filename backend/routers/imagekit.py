...
def upload(file: Annotated[UploadFile, File()], fileName: Annotated[str, Form()]):
    try:
        headers = {
            "Authorization": f"Basic {IMAGEKIT_TOKEN}"
        }
        ...
        response = requests.post(IMAGEKIT_UPLOAD_URL, headers=headers, files=files)
        ...
        if response.ok:
            if response.text:
                ...
                return { "detail": "uploading image successful" }
        else:
    ...
    try:
        ...
        if response.ok:
            if response.text:
                ...
                return { "detail": "deleting image successful" }
        else:
            return JSONResponse(status_code=response.status_code, content=jsonable_encoder({"detail": response.text}))
